...
export default async function ShelterRefundsPage({ searchParams }: PageProps) {
  ...
  const refundRows = await Promise.all(refundDonations.map(async (donation) => {
    let refundWei = donation.amount_wei ? BigInt(donation.amount_wei) : BigInt(0);
    ...
    if (donation.refund_tx_hash) {
      try {
        ...
        const refundLog = receipt.logs.map((log) => {
          try {
        ...
        // Keep the original on-chain donation amount as a display fallback.
      }
    }
