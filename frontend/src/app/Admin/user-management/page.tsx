      ...
      <div className="mx-auto max-w-[1500px] space-y-6">
        ...
        {!isConnected ? <div className="rounded-2xl border border-orange-100 bg-white p-10 text-center font-bold">Connect an authorized admin wallet to continue.</div> : loading ? <div className="grid animate-pulse gap-4 sm:grid-cols-3">{[1,2,3].map((item) => <div key={item} className="h-28 rounded-2xl bg-white" />)}</div> : error ? <div className="rounded-2xl border border-orange-100 bg-white p-8 text-center"><p className="font-black">{error}</p><button onClick={() => void load()} className="mt-4 rounded-full bg-stone-950 px-5 py-2.5 text-sm font-black text-white">Try again</button></div> :
          <section className="overflow-hidden rounded-[1.4rem] border border-orange-100 bg-white shadow-[0_14px_38px_rgba(97,55,17,0.07)]">
            <div className="flex flex-col gap-4 border-b border-orange-100 bg-orange-50/35 p-4 sm:p-5 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-[11px] font-black uppercase tracking-wider text-[var(--color-orange)]">Directory</p><h2 className="mt-1 text-base font-black text-stone-950">Platform users</h2><p className="mt-0.5 text-xs font-medium text-stone-500">{filtered.length} of {users.length} users</p></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-[minmax(18rem,1.5fr)_repeat(3,minmax(9rem,1fr))]"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search name, email, or wallet" className="h-10 rounded-xl border border-orange-100 bg-white px-3 text-sm outline-none transition focus:border-orange-400 focus:ring-2 focus:ring-orange-100" /><select value={role} onChange={(event) => setRole(event.target.value)} className="h-10 rounded-xl border border-orange-100 bg-white px-3 text-sm font-medium outline-none focus:border-orange-400"><option value="all">All roles</option><option value="donor">Donor</option><option value="shelter">Shelter</option><option value="admin">Admin</option></select><select value={status} onChange={(event) => setStatus(event.target.value)} className="h-10 rounded-xl border border-orange-100 bg-white px-3 text-sm font-medium outline-none focus:border-orange-400"><option value="all">All accounts</option><option value="active">Active</option><option value="deactivated">Deactivated</option></select><select value={sort} onChange={(event) => setSort(event.target.value)} className="h-10 rounded-xl border border-orange-100 bg-white px-3 text-sm font-medium outline-none focus:border-orange-400"><option value="newest">Newest</option><option value="oldest">Oldest</option><option value="name-asc">Name A–Z</option><option value="name-desc">Name Z–A</option></select></div></div>
            {filtered.length ? <div className="overflow-x-auto"><table className="w-full min-w-[1150px] text-left"><thead className="border-b border-orange-100 bg-[rgba(var(--color-cream-rgb),0.55)] text-[11px] uppercase tracking-wider text-stone-500"><tr>{["User", "Email", "Wallet", "Role", "Account", "Created", "Actions"].map((label) => <th key={label} className="px-5 py-3 font-black">{label}</th>)}</tr></thead><tbody className="divide-y divide-orange-100">{filtered.map((user) => <tr key={user.id} className="transition hover:bg-orange-50/35"><td className="px-5 py-4"><div className="flex items-center gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[var(--color-peach)] text-sm font-black text-[var(--color-orange)]">{(user.full_name || user.email || "U").slice(0,2).toUpperCase()}</span><p className="max-w-52 truncate font-semibold">{user.full_name || "Unnamed user"}</p></div></td><td className="max-w-56 truncate px-5 py-4 text-sm font-medium text-stone-600">{user.email || "—"}</td><td className="px-5 py-4"><div className="flex items-center gap-2"><span className="font-mono text-xs">{shortWallet(user.wallet_address)}</span>{user.wallet_address ? <button onClick={() => void copyWallet(user.wallet_address!)} className="rounded-lg border border-orange-100 px-2 py-1 text-[10px] font-black text-[var(--color-orange)]">{copied === user.wallet_address ? "Copied" : "Copy"}</button> : null}</div></td><td className="px-5 py-4"><Badge className={user.role === "admin" ? "bg-stone-950 text-white" : user.role === "shelter" ? "bg-orange-100 text-orange-800" : "bg-amber-50 text-amber-800"}>{user.role}</Badge></td><td className="px-5 py-4"><Badge className={user.account_status === "active" ? "bg-emerald-100 text-emerald-700" : "bg-stone-200 text-stone-700"}>{user.account_status}</Badge></td><td className="px-5 py-4 text-xs font-medium text-stone-500">{date(user.created_at)}</td><td className="px-5 py-4"><button onClick={() => setDetails(user)} className="inline-flex items-center gap-2 whitespace-nowrap rounded-lg border border-stone-300 bg-white px-3.5 py-2 text-xs font-bold text-stone-800 shadow-sm transition hover:border-stone-400 hover:bg-stone-50 focus:outline-none focus:ring-2 focus:ring-stone-300 focus:ring-offset-2"><svg viewBox="0 0 24 24" className="h-4 w-4 text-stone-500" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z" /><circle cx="12" cy="12" r="2.5" /></svg>View details</button></td></tr>)}</tbody></table></div> : <div className="p-12 text-center"><h3 className="text-base font-black">No users found</h3><p className="mt-1 text-sm font-semibold text-stone-500">Adjust the search or filters to view another account.</p></div>}
          </section>}
      </div>
    </main>
