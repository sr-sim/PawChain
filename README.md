...
- 🛡️ **RoleNFT verification** for platform administrators, registered donors, and verified shelters.
...
PawChain/
├── backend/
│   ├── contracts/          # RoleNFT, campaign factory, and campaign contracts
│   ├── ignition/           # Local and Sepolia deployment modules
│   └── test/               # Hardhat smart-contract tests
├── frontend/
│   ├── src/app/            # Donor, shelter, admin, and API routes
│   ├── src/lib/            # Blockchain, Supabase, session, and domain logic
...
- A local Hardhat network or supported deployed network
...
Create `frontend/.env.local` and configure the values required by your deployment:
...
For Sepolia deployment, configure the backend environment where applicable:
...
Never commit private keys, service-role keys, app passwords, or wallet-session secrets.

### 3. Run the complete local environment on Windows

```powershell
npm.cmd run dev:all
```

This starts the local Hardhat node, deploys the contracts, seeds role information, and starts the frontend.

Alternatively, run the components separately:
...
Open [http://localhost:3000](http://localhost:3000) after the frontend starts.
...
- **Transparency:** Donors can inspect how donations are recorded, held, released, and refunded.
- **Trust:** Verified shelter roles and auditable transactions reduce uncertainty between donors and recipients.
- **Tracking:** Campaign, milestone, donation, withdrawal, and refund progress remain visible throughout the process.
- **Donor confidence:** Blockchain transaction proofs help donors confirm that contributions reached the intended campaign contract.
- **Accountability:** Evidence review prevents a shelter from progressing to later milestones without accounting for the previous release.
- **Security:** Smart-contract permissions, campaign states, withdrawal restrictions, and reentrancy protection reduce unauthorized fund access.

## 📌 Scope note

PawChain combines on-chain financial enforcement with off-chain application data. Donations, releases, milestone states, role checks, and refunds are enforced or evidenced through smart contracts, while profiles, uploaded documents, campaign descriptions, notifications, reports, and analytics are managed through the Next.js and Supabase application layer.
